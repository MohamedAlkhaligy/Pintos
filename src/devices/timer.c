...
timer_sleep (int64_t ticks) 
{
	if (ticks > 0) {
		...
		// Assign the amount of ticks the thread needs to sleep.
		// sleep_ticks were added in thread struct.
		current_thread -> sleep_ticks = timer_ticks() + ticks;
	
		// Add the thread to the list of blocked threads which is sort-inserted in the list according
		// to its sleep_ticks.
		list_insert_ordered(&blocked_threads, &current_thread -> elem, compare_ticks , 0);
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  while (!list_empty(&blocked_threads)) {
	  struct list_elem *front = list_front(&blocked_threads);
